...
#write table
write.xlsx(ntm_res, file = "supp 表3.中国境内非结核分枝杆菌(NTM)病原检出率(20项研究，排除刘超,王泉.xlsx", asTable = FALSE, overwrite = TRUE)
...
fileName <- "supp 表3.中国境内非结核分枝杆菌(NTM)病原检出率(20项研究，排除刘超,王泉.rtf"
tabName <- "supp 表3.中国境内非结核分枝杆菌(NTM)病原检出率(20项研究，排除刘超,王泉"
...
png(file = "supp 表3.中国境内非结核分枝杆菌(NTM)病原检出率(20项研究，排除刘超,王泉.png", width = 9, height = 10,units = "in", res = 600)
forest(res, xlim=c(-100, 80), at=c(0, 10, 20, 30, 40, 50), 
       ...
       ilab.xpos =c(-35,-15), 
       cex=0.75, ylim=c(-3,39), top=4, order= Regions,
       rows =c(3:4,9:12,17:27,32:34), mlab=mlabfun("RE Model for All Studies", res),
...
text(-100, c(5,13,28,35), pos=4, c("Northeast"
                               ,
                               "Central"
                               ,
                               "Southeast"
...
addpoly(res.sw, row=30.5, transf = mytransf, targ = list(ni=1/(res.sw$se)^2),mlab=mlabfun("RE Model for Southwest", res.sw))
