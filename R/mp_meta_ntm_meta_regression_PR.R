...
ntm <- read_excel("MP meta分析数据收集表核查prev_eng.xlsx",skip = 1) %>%
  ...
  ies.da <- escalc(xi = NumbervalidateNTM, ni = ScreeningPatientNumber, data = da, add = 0, digits = 10, measure = "PR")
  ...
  regplot(m_reg, xlim=c(2016,2025), ylim = c(0,0.5), xlab="Publication Year", 
          ylab = "NTM isolation rate (%)", yaxt = "n",
          digits=1, las=1, bty="l",
          offset=c(1.6,0.8), labsize=0.9, psize = 0.7, pch = 2)
  axis(side=2, at=seq(0.0, 0.5,0.1), labels=seq(0,50,10))
  text(2016, 0.3, pos = 4, paste0("y = ", round(m_reg$b[1],4), "+", round(m_reg$b[2],4), "*year"))
