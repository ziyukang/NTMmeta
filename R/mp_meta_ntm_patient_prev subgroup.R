...
pes.summary <- metaprop(event = NTMpatient, n = ScreeningPatientNumber,
                        ...
                        backtransf = TRUE, targs = list(ni=1/res$se^2), 
                        pscale = 1, byvar = Regions, tau.common = F, 
                        #tau.preset = sqrt(res$tau2)
                        ) 
png(file = "NTM患病率亚组分析(exclude李红玲2023).png", width = 8, height = 7,units = "in", res = 300)
forest(pes.summary, 
       sortvar = TE,
       resid.hetstat = FALSE ,
       common = FALSE, 
...
png(file = "NTM患病率亚组分析.png", width = 8, height = 7,units = "in", res = 300)
forest(pes.summary, 
       sortvar = TE,
       common = FALSE, 
       print.tau2 = TRUE, 
...
#da$Trial <- "MP"
#res <- rma(yi, vi, mods = ~ Regions, random = ~1|Regions, data=da)
