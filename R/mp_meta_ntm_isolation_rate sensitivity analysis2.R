...
ntm <- read_excel("MP meta分析数据收集表核查prev.xlsx",skip = 1) %>%
  select(FirstAuthor, PublicationYear, StudyDesign, 
         StudyRegion, Regions, ScreeningPatientNumber, 
         prevalence, 总计) %>%
         filter(!FirstAuthor == "刘超") %>%
         filter(!FirstAuthor == "李红玲") %>%
         filter(!FirstAuthor == "李观强") %>%
         filter(!FirstAuthor == "王泉") %>%
  mutate(NumbervalidateNTM = as.numeric(总计)) %>%
  mutate(Regions = factor(Regions, levels = c("东北","中部","东南","西南"))) %>%
...
#write table
write.xlsx(ntm_res, file = "Supp 表2.中国境内非结核分枝杆菌(NTM)病原检出率(18项研究，排除刘超,李红玲,李观强,王泉).xlsx", asTable = FALSE, overwrite = TRUE)
...
fileName <- "Supp 表2.中国境内非结核分枝杆菌(NTM)病原检出率(18项研究，排除刘超,李红玲,李观强,王泉).rtf"
tabName <- "Supp 表2.中国境内非结核分枝杆菌(NTM)病原检出率(18项研究，排除刘超,李红玲,李观强,王泉)"
col_header <- "地区|研究数量|NTM流行率|n/N|研究异质性|异质性p值|Egger's检验t值|Egger's检验p值"
...
png(file = "Supp 表2.中国境内非结核分枝杆菌(NTM)病原检出率(18项研究，排除刘超,李红玲,李观强,王泉).png", width = 9, height = 10,units = "in", res = 600)
forest(res, xlim=c(-100, 80), at=c(0, 10, 20, 30, 40, 50), 
       ...
       ilab.xpos =c(-35,-15), 
       cex=0.75, ylim=c(-3,37), top=4, order= Regions,
       rows =c(3:4,9:12,17:25,30:32), mlab=mlabfun("RE Model for All Studies", res),
...
text(-100, c(5,13,26,33), pos=4, c("Northeast"
                               ,
                               "Central"
                               ,
                               "Southeast"
...
addpoly(res.sw, row=28.5, transf = mytransf, targ = list(ni=1/(res.sw$se)^2),mlab=mlabfun("RE Model for Southwest", res.sw))
addpoly(res.se, row= 15.5, transf = mytransf, targ = list(ni=1/(res.se$se)^2),mlab=mlabfun("RE Model for Southeast", res.se))
addpoly(res.ce, row= 7.5, transf = mytransf, targ = list(ni=1/(res.ce$se)^2), mlab=mlabfun("RE Model for Central", res.ce))
