...
ntm <- read_excel("MP meta分析数据收集表核查prev_eng.xlsx",skip = 1) %>%
  select(FirstAuthor0, FirstAuthor, PublicationYear, StudyDesign, 
         ...
         filter(!FirstAuthor0 == "李红玲") %>%
         filter(!FirstAuthor0 == "李观强") %>%
  mutate(NumbervalidateNTM = as.numeric(总计)) %>%
  mutate(Regions = factor(Regions, levels = c("东北","西北","中部","东南","西南"),
                          labels = c("Northeast", "Northwest","Central","Southeast","Southwest"))) %>%
  ...
  ies.da <- escalc(xi = unlist(ntm$NumbervalidateNTM), ni = ntm$ScreeningPatientNumber, add = 0, digits = 10, measure = "PFT")
  ies.da <- cbind(ntm, ies.da)

  #make forest plot using metaprop and forest.meta
  ...
  png(file = "Figure S1.NTM isolation rate in China subgroup analysis(19 studies，exclude Liu,Li,Li).png", width = 9, height = 10,units = "in", res = 300)
  forest(pes.summary, 
         sortvar = TE,
         common = FALSE, 
         print.tau2 = TRUE, 
  ...
  for(region in c("所有地区",unique(as.character(ntm$Regions)))){
    ...
    if(region == "所有地区"){
      da <- ies.da
      pes.da <- rma(yi, vi, data =da , transf = transf.ipft.hm, targ = list(ni=1/(pes.da$se)^2),method = "REML", weighted = TRUE)
...
#write table
write.xlsx(ntm_res, file = "Supp Table 1.NTM isolation rate in China subgroup analysis(19 studies，exclude Liu,Li,Li).xlsx", asTable = FALSE, overwrite = TRUE)

tab <- ntm_res %>% 
  select(地区, num_study, prev_NTM, n_N, I2, Q_pval, egger.t, egger.pval) %>%
  mutate_if(is.character, stri_enc_toutf8)
for(i in 1:nrow(tab)){
  tab[i,1] <- r2rtf::utf8Tortf(tab[i,1])
}	

fileName <- "Supp Table 1.NTM isolation rate in China subgroup analysis(19 studies，exclude Liu,Li,Li).rtf"
tabName <- "Supp Table 1.NTM isolation rate in China subgroup analysis(19 studies，exclude Liu,Li,Li)"
col_header <- "Region|Number of study|NTM detection rate|n/N|Cross study heterogeneity|Heterogeneity p value|Egger's test t|Egger's test p value"
...
png(file = "Supp Figure S1.NTM isolation rate in China subgroup analysis(19 studies，exclude Liu,Li,Li).png", width = 9, height = 10,units = "in", res = 600)
forest(res, xlim=c(-100, 80), at=c(0, 10, 20, 30, 40, 50), 
       ...
       rows =c(3:4,9,14:17,22:30,35:37), mlab=mlabfun("RE Model for All Studies", res),
...
addpoly(res.sw, row=33.5, transf = mytransf, targ = list(ni=1/(res.sw$se)^2),mlab=mlabfun("RE Model for Southwest", res.sw))
